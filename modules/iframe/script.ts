 ...
 * Uses an IntersectionObserver to lazily activate iframes as they scroll into
 * view, hides the loading indicator once the iframe has loaded, and applies
 * iframe-resizer so the iframe height tracks its content.
 *
 * @returns void
 */
const init = (): void => {
  const modules = document.querySelectorAll<HTMLElement>('[data-module="iframe"]');
